// https://github.com/oranzhang/mina-webpack-plugin/blob/25c2d5a5f4c94f6a3e3f5d0d2e0a0a7ea6ea5702/lib/index.js
...
module.exports = class MinaWebpackPlugin {
    ...
    compiler.hooks.emit.tapAsync(PLUGIN_NAME, (compilation, callback) => {
      ...
      for (const name of this.appEntries) {
        ...
        if (fsExtra.existsSync(file)) {
          data = fsExtra.readJsonSync(file)
        }
  ...
  async getComponents (components, instance) {
    const infoFilePath = `${instance}.json`
    if (!fsExtra.existsSync(infoFilePath)) return
    const { usingComponents = {} } = await fsExtra.readJson(infoFilePath)
